...
def reset_community_data():
    ...
    # 清理测试用户
    User.objects.filter(username__in=['marketer001', 'designer001']).delete()
...
大家有什么新的SEO策略可以分享？''',
    ...
    print('\n社区数据初始化完成！')
    print(f'创建了 {Discussion.objects.count()} 个讨论')
    print(f'创建了 {Comment.objects.count()} 条评论')
    print(f'创建了 {Like.objects.count()} 个点赞')
    print(f'创建了 {Tag.objects.count()} 个标签')

if __name__ == '__main__':
    reset_community_data() 